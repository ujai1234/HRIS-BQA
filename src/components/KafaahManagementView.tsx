...
export const KafaahManagementView: React.FC = () => {
  ...
  return (
    <div id="kafaah-management-view" className="space-y-5">
      ...
      <div className="bg-white rounded-[16px] border border-slate-200 p-5 shadow-xs">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-[#051F20] tracking-tight">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-50 hover:bg-slate-100 text-[#051F20] text-xs font-bold uppercase tracking-wider rounded-[10px] border border-slate-200 transition-colors cursor-pointer"
            >
              <SlidersHorizontal className="w-3.5 h-3.5 text-[#163832]" />
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-slate-50 text-[#051F20] text-xs font-bold uppercase tracking-wider rounded-[10px] border border-slate-200 transition-colors cursor-pointer"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5 pt-4 border-t border-slate-100">
          <div className="bg-slate-50/70 p-3 rounded-[12px] border border-slate-100">
            <div className="text-[10px] font-bold text-[#8EB69B] uppercase tracking-wider">Rata-rata Kafa'ah / JP</div>
            <div className="text-lg font-bold text-[#051F20] mt-0.5">{formatRupiah(stats.avgRate)}</div>
            <div className="text-[10px] text-slate-400 mt-0.5">Standar per jam mengajar</div>
          </div>

          <div className="bg-slate-50/70 p-3 rounded-[12px] border border-slate-100">
            <div className="text-[10px] font-bold text-[#8EB69B] uppercase tracking-wider">Rata-rata Gaji Pokok</div>
            <div className="text-lg font-bold text-[#051F20] mt-0.5">{formatRupiah(stats.avgBase)}</div>
            <div className="text-[10px] text-slate-400 mt-0.5">Kehadiran pokok bulanan</div>
          </div>

          <div className="bg-slate-50/70 p-3 rounded-[12px] border border-slate-100">
            <div className="text-[10px] font-bold text-[#8EB69B] uppercase tracking-wider">Rata-rata Transport</div>
            <div className="text-lg font-bold text-[#051F20] mt-0.5">{formatRupiah(stats.avgTransport)}</div>
            <div className="text-[10px] text-slate-400 mt-0.5">Uang kehadiran harian</div>
          </div>

          <div className="bg-[#DAF1DE]/40 p-3 rounded-[12px] border border-[#8EB69B]/30">
            <div className="text-[10px] font-bold text-[#163832] uppercase tracking-wider">Estimasi Anggaran Kafa'ah</div>
            <div className="text-lg font-bold text-[#051F20] mt-0.5">{formatRupiah(stats.totalEstimatedOutlay)}</div>
            <div className="text-[10px] text-[#163832]/70 mt-0.5">Proyeksi 1 bulan (Jadwal aktif)</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="text-[10px] font-bold uppercase tracking-wider text-[#8EB69B] self-center sm:ml-auto">
          Menampilkan <strong className="text-[#051F20]">{filteredTeachers.length}</strong> guru
        </div>
      </div>
              ...
              {filteredTeachers.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-10 text-center text-slate-400">
                    <Coins className="w-8 h-8 mx-auto mb-1.5 text-slate-300" />
                    <p className="font-bold text-[#8EB69B] text-xs uppercase tracking-wider">Tidak ada data guru yang sesuai pencarian.</p>
                  </td>
                </tr>
              ) : (
                filteredTeachers.map((teacher, idx) => {
                  ...
                  return (
                    <tr key={teacher.id} className={`hover:bg-slate-50 transition-colors ${isDraftDirty ? 'bg-[#DAF1DE]/40' : ''}`}>
                      {/* Number */}
                      <td className="py-3 px-3 text-center text-[#8EB69B] font-mono font-medium">
                        {idx + 1}
                      </td>

                      {/* Name & NIP */}
                      <td className="py-3 px-4">
                        <div className="font-bold text-[#051F20]">{teacher.name}</div>
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className="text-[10px] font-mono font-semibold text-[#8EB69B]">
                            {teacher.nip}
                          </span>
                          <span className="text-[10px] font-bold text-[#163832] bg-[#DAF1DE] px-1.5 py-0.5 rounded-[4px] uppercase tracking-wider">
                            {teacher.unit}
                          </span>
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="inline-flex flex-col items-center">
                          <span className="font-bold text-[#051F20] text-xs">
                            {load.totalHours} <span className="text-[10px] text-slate-400">JP/mg</span>
                          </span>
                          <span className="text-[10px] font-semibold text-[#8EB69B]">
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="relative flex items-center justify-end">
                          <span className="text-[10px] font-bold text-[#8EB69B] mr-1 select-none">Rp</span>
                          <input
                            type="text"
                            ...
                            className="w-28 text-right py-1 px-2 text-xs font-bold bg-white border border-slate-200 rounded-md focus:ring-2 focus:ring-[#163832] focus:border-[#051F20] focus:outline-none text-[#051F20]"
                          />
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="relative flex items-center justify-end">
                          ...
                          <input
                            ...
                            className="w-24 text-right py-1 px-2 text-xs font-bold bg-white border border-slate-200 rounded-md focus:ring-2 focus:ring-[#163832] focus:border-[#051F20] focus:outline-none text-[#163832]"
                          />
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="relative flex items-center justify-end">
                          ...
                          <input
                            ...
                            className="w-24 text-right py-1 px-2 text-xs font-bold bg-white border border-slate-200 rounded-md focus:ring-2 focus:ring-[#163832] focus:border-[#051F20] focus:outline-none text-[#051F20]"
                          />
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 text-right font-mono">
                        <div className="font-bold text-[#051F20]">
                          {formatRupiah(estimatedMonthly)}
                        </div>
                        <div className="text-[10px] font-semibold text-[#8EB69B]">
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {isDraftDirty ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-[#163832] hover:bg-[#051F20] text-[#DAF1DE] rounded-[8px] text-[10px] font-bold uppercase tracking-wider transition-colors shadow-xs cursor-pointer"
        ...
        <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-5 flex-1 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -transtone-y-1/2 font-semibold text-slate-400 dark:text-slate-500 text-xs">Rp</span>
              <input
                type="text"
                value={baseSalaryStr}
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800">
            ...
            {updateHourly && (
              <div className="pl-6 space-y-1">
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -transtone-y-1/2 text-slate-400 dark:text-slate-500 text-xs font-semibold">Rp</span>
                  <input
                    type="text"
                    value={hourlyRateStr}
                    onChange={(e) => setHourlyRateStr(formatCurrencyInput(e.target.value))}
