  ...
  return (
    <div className="bg-white dark:bg-[#0B2B26] rounded-[16px] border border-slate-200/80 dark:border-[#163832] shadow-xs p-5">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-9 h-9 rounded-xl bg-blue-50 dark:bg-[#163832] text-blue-600 dark:text-[#8EB69B] flex items-center justify-center border border-blue-200/60 dark:border-[#0B2B26] shrink-0">
          <PenTool className="w-4 h-4" />
        </div>
        <div>
          <h3 className="font-bold text-slate-900 dark:text-[#DAF1DE] text-sm">Jurnal Pekerjaan Harian</h3>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-50 dark:bg-[#163832] border border-slate-200 dark:border-[#0B2B26] rounded-[12px] p-3 text-sm focus:ring-2 focus:ring-[#8EB69B] focus:border-transparent dark:text-[#DAF1DE] min-h-[80px]"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-[#8EB69B] mb-1">
            ...
            <div>
              ...
              {!photoUrl ? (
                ...
                >
                  ...
                  <span className="text-xs text-slate-500 dark:text-[#8EB69B]">Klik untuk memilih foto (Max: 5MB)</span>
                  <input 
                    type="file" 
                    accept="image/*" 
                    className="hidden" 
        ...
        <div>
          ...
          <div className="space-y-3 max-h-[38rem] overflow-y-auto pr-2">
            {todayJournals.length === 0 ? (
              <p className="text-xs text-slate-400 dark:text-[#8EB69B] text-center py-4 italic">Belum ada jurnal hari ini</p>
            ) : (
              todayJournals.map(journal => (
                <div key={journal.id} className="bg-slate-50 dark:bg-[#163832] p-4 rounded-[12px] border border-slate-100 dark:border-[#0B2B26] space-y-3">
                  <div>
                    <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-[#8EB69B] mb-1">
                      <ListChecks className="w-3.5 h-3.5 text-blue-500 dark:text-[#DAF1DE]" /> Hari Ini:
                    </span>
                    <p className="text-sm text-slate-600 dark:text-[#DAF1DE] bg-white dark:bg-[#0B2B26] p-2 rounded-lg border border-slate-100 dark:border-[#163832]">
                      {journal.taskToday}
                    </p>
                  </div>
                  <div>
                    <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-[#8EB69B] mb-1">
                      <Clock className="w-3.5 h-3.5 text-amber-500 dark:text-[#DAF1DE]" /> Rencana Besok:
                    </span>
                    <p className="text-sm text-slate-600 dark:text-[#DAF1DE] bg-white dark:bg-[#0B2B26] p-2 rounded-lg border border-slate-100 dark:border-[#163832]">
                  ...
                  {journal.photoUrl && (
                    <div className="mt-2 relative rounded-lg overflow-hidden border border-slate-200 dark:border-[#163832]">
